...
### Needed to enable Auth in staging/production
...
| `TOTP_ENCRYPTION_KEY`  | Generate yourself    | Separate 32+ character secret used to encrypt admin TOTP seeds at rest. |
...
| `INTERNAL_API_KEY`      | Generate yourself       | Protects the optional internal AI queue endpoint; use 32+ random characters.   |

### Optional content-library integrations
...
| Variable            | Where to get it               | Notes                                                                                                 |
| ------------------- | ----------------------------- | ----------------------------------------------------------------------------------------------------- |
| `EXPO_ACCESS_TOKEN` | Expo project settings         | Optional unless Expo push access-token security is enabled. Expo push is the implemented mobile path. |
| `FCM_PROJECT_ID`    | Firebase Console              | Reserved for a future direct-native FCM adapter; not needed when the Expo provider is used.           |
...
| `APNS_TEAM_ID`      | Apple Developer               | Reserved for a future direct-native APNs adapter; not needed when the Expo provider is used.          |
...
- Production startup rejects the development JWT secret or any access secret shorter than 32 characters.
- Privileged Admin/Content Editor routes require a JWT issued after successful 2FA verification.
